...
const QuoteOutput = ({ data }) => {
  ...
  if (typeof data === 'string') content = data;
    ...
    if (data.alignment && typeof data.alignment === 'string') style = 'text-'+data.alignment;
  }

  if (!content) return '';
  return (
    <div className="h-full bg-gray-200 p-4 rounded-xl mx-4 my-8 text-gray-700">
      ...
      <a className={"block "+style}>
        <span className="text-gray-500 text-sm">{ ReactHtmlParser(caption) }</span>
      </a>
    </div>
  )
