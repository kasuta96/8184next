...
const TransOutput = ({ data, count }) => {
  ...
  if (data.original && typeof data.original === 'string') original = data.original
  if (data.translation && typeof data.translation === 'string') translation = data.translation
  ...
  const Tabs = ({ id = "", original, translation }) => {
    ...
    return (
      <div
        ...
        data-trans={id}
      >
        <div className="float-right text-indigo-300 pl-2">
          <svg width="20" height="20" fill="currentColor" viewBox="0 0 469.333 469.333" xmlns="http://www.w3.org/2000/svg">
        ...
        <div data-lang="orig" className={openTrans == false ? 'block' : 'hidden'}>
          <Tag data={original} />
        </div>
        <div data-lang="tran" className={openTrans == true ? 'block' : 'hidden'}>
